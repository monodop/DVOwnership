...
namespace DVOwnership
{
    ...
    {
        public static JobChainController GenerateLoadHaulUnloadChainJobForCars(System.Random rng, List<List<Car>> carSetsForJob, CargoGroup cargoGroup, StationController originController)
        {
            ...
            var loadMachinesThatSupportCargoType = originController.logicStation.yard.GetWarehouseMachinesThatSupportCargoTypes(cargoTypes);
            if (loadMachinesThatSupportCargoType.Count == 0)
            {
            ...
            loadMachinesThatSupportCargoType.RemoveAll((machine) => machine.WarehouseTrack.length < (double)approxLengthOfWholeTrain);
            if (loadMachinesThatSupportCargoType.Count == 0)
            {
            ...
            var loadMachine = Utilities.GetRandomFrom(rng, loadMachinesThatSupportCargoType);
            ...
            var unloadMachinesThatSupportCargoType = destinationController.logicStation.yard.GetWarehouseMachinesThatSupportCargoTypes(cargoTypes);
            if (unloadMachinesThatSupportCargoType.Count == 0)
            {
            ...
            unloadMachinesThatSupportCargoType.RemoveAll((machine) => machine.WarehouseTrack.length < (double)approxLengthOfWholeTrain);
            if (unloadMachinesThatSupportCargoType.Count == 0)
            {
            ...
            var unloadMachine = Utilities.GetRandomFrom(rng, unloadMachinesThatSupportCargoType);

            var gameObject = new GameObject($"ChainJob[{JobType.ComplexTransport}]: {originController.logicStation.ID} - {destinationController.logicStation.ID}");
